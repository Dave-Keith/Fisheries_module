...
A.  Your job is to design an annual trawl survey for Dusky Scallop Shark (*Dustious maximus*; Figure \@ref(fig:base-plt)).  Simple background:
  - The survey occurs June when the stock is spawning, your survey vessel is available all month
  - From start to finish it takes approximately 3 hours to complete a tow and finish the scientific analyses
  - For this example we *know* there are 100,000 tonnes of biomass located in the survey domain (*in the real world you would not know this!!!*)
...
C.  In planning your survey there are 2 variables to consider
    a. You can vary the number of survey tows
    b. The example provides results for 3 different types of survey designs
       i.   Random survey stations
       ii.  Stratified by NAFO sub-area (Figure \@ref(fig:nafo-plt))
       iii. Stratified by Depth strata (Figure \@ref(fig:depth-plt))
...
5. How many survey stations would you recommend?
   - Consider the 'constraints' on your available time in Part "A" of the Tutorial Outline above.
  
# The Tutorial Tutorial

i.    Open the file "Survey_tutorial.Rmd" in R-Studio
ii.   On line 75 of this file you can change the number of tows used in the survey.  Default is 20 tows
...
Optional additional exploration if this is all really making perfect sense to you (in the 'chunk' starting on line 85)!
    - You can change the number of realizations run on line 94
      - I suggest trying 1, 5, and 250 (this will be slow!) to see if/how your opinion changes with more realizations of your data.
    - You can change how the biomass is distributed across the survey area on lines 96-102
...
tot.biomass <- 1e5 # Could be any units, let's call it tonnes, and because of how I have this set up below this number isn't exact
...
# You can change how the simulation distributes the biomass in the simulation through space.  This is generally too much!!
...
req.packages <- c("tidyverse","lubridate","plotly","sf","sp","data.table","units","cowplot","knitr",'concaveman',
                  'ggthemes',"nngeo","marmap","geoR","ggplot2","stars","tmaptools","rnaturalearth","zipfR",
...
if(bm.dist == "Random") 
{
  gmrf <- grf(cov.pars = c(1, 2), grid=data.frame(cent.split)[,1:2],messages=F)
  gmrfs.pnts <- as.data.frame(gmrf$coords)
  gmrfs.pnts$variable1 <- gmrf$data
 }
...
if(bm.dist == "NAFO") 
{
  ...
  # 
  gmrf <- grf(cov.pars = c(1, 1), grid=data.frame(cent.split)[,1:2],messages=F)
  gmrfs.pnts <- as.data.frame(gmrf$coords)
  gmrfs.pnts$variable1 <-   gmrf$data + cent.eff$effect # 
  gmrfs.pnts$strata = cent.eff$strata.y
}
...
gmrf.pnts.sf <- st_as_sf(gmrfs.pnts, coords = c('lat','long'),crs = 4326)
...
b.quants <- res %>% group_by(survey,metric) %>% dplyr::summarise(med = quantile(biomass,probs = c(0.5),na.rm=T),
                                                                 q25 = quantile(biomass,probs = c(0.25),na.rm=T),
                                                                 q75 = quantile(biomass,probs = c(0.75)),na.rm=T)
p.bm <- ggplot(b.quants) + geom_point(aes(y=med,x = survey)) + facet_wrap(~metric,scale='free_y',ncol = 1) +
                ...
                geom_hline(aes(yintercept = h.lines),color='grey',linetype='dashed',linewidth=2) +
                theme_bw()
p.bm
}
