...
ii.   On line 111 of this file you can change the number of tows used in the survey.  Default is 200 tows
iii.  On line 115-116 of this file you can change the survey stratification scheme, options are "Random" and "NAFO"
...
# So I need to loop these through years.
gmrfs <- NULL
rec.gmrfs <- NULL
for(i in 1:n.years)
{
gmrf.tmp <- grf(cov.pars = c(1, 1), grid=data.frame(cent.split)[,1:2],messages=F)
rec.gmrf.tmp <- grf(cov.pars = c(0.4, 0.5), grid=data.frame(cent.split)[,1:2],messages=F)
gmrfs.tmp <- as.data.frame(gmrf.tmp$coords)
gmrfs.tmp$variable1 <- gmrf.tmp$data
gmrfs.tmp$years <- yrs[i]
rec.gmrfs.tmp <- as.data.frame(rec.gmrf.tmp$coords)
rec.gmrfs.tmp$variable1 <- rec.gmrf.tmp$data
rec.gmrfs.tmp$years <- yrs[i]

gmrfs[[i]] <- gmrfs.tmp
rec.gmrfs[[i]] <- rec.gmrfs.tmp
}

gmrfs <- do.call("rbind",gmrfs)
rec.gmrfs <- do.call("rbind",rec.gmrfs)

gmrfs.sf <- st_as_sf(gmrfs, coords = c('lat','long'),crs = 4326)
rec.gmrfs.sf <- st_as_sf(rec.gmrfs, coords = c('lat','long'),crs = 4326)
